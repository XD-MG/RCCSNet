...
@UDA.register_module()
class BiHCSC(UDADecorator):

    def __init__(self, **cfg):
        super(BiHCSC, self).__init__(**cfg)
        # self.img_from = "src"
        # basic setup
        self.local_iter = 0
        ...
        # self.crop_size = cfg.get('crop_size', (512, 512))
        ...
        if self.contrast_mode == 'multiple_select':
            ...
            self.feat_distributions = ProtoEstimator(dim=self.proj_dim, class_num=self.num_classes,
                                                     memory_length=self.memory_length)
            self.feat_distributions_tgt = ProtoEstimator(dim=self.proj_dim, class_num=self.num_classes,
                                                     memory_length=self.memory_length)
    ...
    def random_crop(self, image, gt_seg, prod=True):
        ...
        gt_seg = gt_seg
        res_img, res_gt = [], []
        for img, gt in zip(image, gt_seg): #img:512x512x3  gt:512x512
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas, target_gt_semantic_seg):
        ...
        ema_target_logits = self.get_ema_model().encode_decode("tgt_re", weak_target_img, target_img_metas)#(2,6,512,512)
        ...
        # update src_distribution
        ema_src_feat = self.get_ema_model().extract_auxiliary_feat("src_re_update", weak_img)
        mean = {}
        covariance = {}
        bank = {}
        if self.contrast_mode == 'multiple_select':
        ...
        source_losses = self.get_model().forward_train("src_re", img, img_metas, gt_semantic_seg, return_feat=False,
        ...
        if self.local_iter >= self.start_distribution_iter:
            ...
            # update tgt_distribution
            weak_tgt_gt_seg = pseudo_label.unsqueeze(1).clone().detach()
            
            ema_tgt_feat = self.get_ema_model().extract_auxiliary_feat("tgt_re_update", weak_target_img)
            tgt_mean = {}
            tgt_covariance = {}
            tgt_bank = {}
            if self.contrast_mode == 'multiple_select':
                for idx in range(len(self.calc_layers)):
                    feat, mask = contrast_preparations(ema_tgt_feat[idx], weak_tgt_gt_seg, self.enable_avg_pool,
                                                    self.scale_min_ratio, self.num_classes, self.ignore_index)
                    self.feat_distributions_tgt[idx].update_proto(features=feat.detach(), labels=mask)
                    tgt_mean[idx] = self.feat_distributions_tgt[idx].Ave
                    tgt_covariance[idx] = self.feat_distributions_tgt[idx].CoVariance
                    tgt_bank[idx] = self.feat_distributions_tgt[idx].MemoryBank
            else:  # 'resize_concat' or None
                feat, mask = contrast_preparations(ema_tgt_feat, weak_tgt_gt_seg, self.enable_avg_pool,
                                                self.scale_min_ratio, self.num_classes, self.ignore_index)
                self.feat_distributions_tgt.update_proto(features=feat.detach(), labels=mask)
                tgt_mean = self.feat_distributions_tgt.Ave
                tgt_covariance = self.feat_distributions_tgt.CoVariance
                tgt_bank = self.feat_distributions_tgt.MemoryBank
            ...
            target_inter_losses = self.get_model().forward_train("tgt_re", target_img, target_img_metas, pseudo_lbl, return_feat=False,
                                                           mean=mean, covariance=covariance, bank=bank, mode='aux')
            target_inter_loss, target_inter_log_vars = self._parse_losses(target_inter_losses)
            log_vars.update(add_prefix(target_inter_log_vars, 'tgt_inter'))
            target_inter_loss.backward()


            #src_inter_cl
            source_inter_losses = self.get_model().forward_train("src_re", img, img_metas, gt_semantic_seg, return_feat=False,
                                                    mean=tgt_mean, covariance=tgt_covariance, bank=tgt_bank, mode='aux')
            source_inter_loss, source_inter_log_vars = self._parse_losses(source_inter_losses)
            log_vars.update(add_prefix(source_inter_log_vars, 'src_inter'))
            source_inter_loss.backward()
        ...
        if local_enable_self_training:
            ...
            # mix_masks = get_class_masks(pseudo_prob, pseudo_label, self.num_classes, gt_semantic_seg)
            ...
            mix_losses = self.get_model().forward_train("mix_re", mixed_img, img_metas, mixed_lbl, pseudo_weight,
